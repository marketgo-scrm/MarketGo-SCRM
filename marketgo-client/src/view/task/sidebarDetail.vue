<template>
    <div class="todo_detail">
        <van-nav-bar title="代办任务" />
        <van-cell-group class="info">
            <img :src="'data:image/Jpeg;base64,' + imgBase64" alt=""><span class="desc">{{ "name"}}有关的代办任务</span>
        </van-cell-group>
        <van-cell-group style="background: transparent;">
            <div class="top">
                <div style="display: flex;justify-content: space-between;">
                    <span class="title">{{ detail.taskName ? detail.taskName : '' }}</span><span @click="finish"
                        class="status">标记为完成</span>
                </div>
                <div class="date"><span>{{detail.planTime+' '}}前完成</span></div>
            </div>
            <div class="content">
                <div class="text-wrapper">
                    <div class="txt"> {{ contentTxt&& contentTxt[0].text.content || '' }}</div>
                    <div class="txt-btns"> <van-button plain type="primary" size="mini"
                            @click="toToDo(user)">立即发送</van-button>
                        <van-button plain type="primary" size="mini" @click="toToDo(user)">立即发送</van-button>
                    </div>
                </div>
                <div class="image-wrapper">
                    <div class="imgs">
                        <img :src="'data:image/Jpeg;base64,' + imgBase64" alt="">
                    </div>
                    <div class="btns">
                        <van-button plain type="primary" size="mini" @click="toToDo(user)">直接发送</van-button>
                    </div>
                </div>
            </div>
        </van-cell-group>
    </div>
</template>

<script>
import { showSuccessToast } from 'vant';
import 'vant/es/toast/style';
import { welcom } from '../../api/welcom'
import { welcomPinia } from '../../pinia/'
export default {
    data() {
        return {
            imgBase64: ""
            , detail: {
                corpId: "wwa67b5f2bf5754641",
                agentId: "1000004",
                projectUuid: "5bc86e071c6911ed8b4a525400fe9bac",
                chatType: "SINGLE",
                uuid: null,
                taskUuid: "ac5ec1b8f01e4013825baad347633758",
                taskName: "单次立即-发送文本",
                externalUserId: null,
                sender: null,
                attachments: [
                    {
                        msgType: "text",
                        image: null,
                        link: null,
                        miniProgram: null,
                        video: null,
                        file: null,
                        text: {
                            content: "单次立即-发送文本"
                        }
                    }
                ],
                targetType: "MINUTE",
                targetTime: 30,
                planTime: "2023-02-14 19:57:00",
                messageType: "SEND_MESSAGE"
            },

        };
    },
    created() {
        const query = this.$route.query
        welcom.taskDetail(query).then(res => {
            res = {
                code: 0,
                message: "success",
                data: {
                    corpId: "wwa67b5f2bf5754641",
                    agentId: "1000004",
                    projectUuid: "5bc86e071c6911ed8b4a525400fe9bac",
                    chatType: "SINGLE",
                    uuid: null,
                    taskUuid: "ac5ec1b8f01e4013825baad347633758",
                    taskName: "单次立即-发送文本",
                    externalUserId: null,
                    sender: null,
                    attachments: [
                        {
                            msgType: "text",
                            image: null,
                            link: null,
                            miniProgram: null,
                            video: null,
                            file: null,
                            text: {
                                content: "单次立即-发送文本"
                            }
                        }
                    ],
                    targetType: "MINUTE",
                    targetTime: 30,
                    planTime: "2023-02-14 19:57:00",
                    messageType: "SEND_MESSAGE"
                }
            }
            this.detail = res.data || {}
            const pinia = welcomPinia()
            pinia.setDetail(this.detail)
        })
    },

    methods: {
        finish() {
            const query = { corp_id }
            const params = {
                externalUserId: '',//sdk单独获取
                memberId: '',
                sentTIme: Date.now(),
                status: 'DELIVERED',
                taskUuid: this.detail.query.task_uuid,
                type: 'EXTERNAL_USER', // EXTERNAL_USER 客户
                uuid: this.detail.query.uuid,
            }
            welcom.changeStatus({ query, params }).then(res => {
                if (res.code === 0) {
                    showSuccessToast('成功');
                }
            })

        },
    },
    computed: {
        receiverList() {
            const users = this.detail.taskList && this.detail.taskList[0] && this.detail.taskList[0].externalUserId || false
            return users
        },
        contentTxt() {
            const txt = this.detail.attachments && this.detail.attachments.filter(item => item.msgType === 'text') || []

            return txt.length > 0 ? txt : false
        },
        imgList() {
            const imgList = this.detail.attachments && this.detail.attachments.filter(item => item.msgType === 'image') || []
            return imgList.length > 0 ? imgList : false
        }
    }
};
</script>

<style lang="less" scoped>
.todo_detail {
    font-family: 'PingFang SC';
    background: #F5F7FA; //#D8D8D8;
    color: #333333;
    margin-bottom: 100px;

    /deep/.van-nav-bar__content {
        background: #679BFF;

        .van-nav-bar__title {
            color: #fff;
        }
    }

    .info {
        background: #fff;
        padding: 15px;
        margin-bottom: 20px;
        display: flex;

        .desc {
            font-style: normal;
            font-weight: 600;
            color: #333333;
            display: inline-block;
            padding-left: 10px;
        }

        img {
            width: 36px;
        }
    }

    .top {
        background: #fff;
        padding: 15px 15px 0;

        .title {
            font-style: normal;
            font-weight: 600;
            color: #333333;
            display: inline-block;
            width: calc(100% - 80px);
        }

        .status {
            color: #679BFF;
            font-style: normal;
            font-weight: 400;
            font-size: 14px;
        }

        .date {
            font-style: normal;
            font-weight: 400;
            line-height: 24px;
            color: #FF993C;
            padding-top: 20px;
            font-size: 14px;
        }
    }

    .content {
        background: #fff;
        padding: 15px;

        .text-wrapper {
            margin-top: 10px;

            .txt {
                padding: 10px;
                background: #F5F7FA;
                min-height: 60px;
            }

            .txt-btns {
                margin-top: 10px;
                text-align: right;
                button{
                    &:not(:first-child){
                        margin-left: 20px;
                    }
                }
            }
        }


        .image-wrapper {
            margin-top: 10px;
            display: flex;
            justify-content: space-between;
            align-items: center;
        }


    }
}
</style>
