 ...
 * Describe:
 */
public class TestCallBackCorpTagEventService {


    private static String sendCorpTagEvent(String changeType) {
        ...
        try {
            ...
            weComXmlMessage.setEvent("change_external_tag");
            weComXmlMessage.setId("etqPhANwAA-yF9aOY1iwOlKIxmZpHreg");
            weComXmlMessage.setTagType("tag");
            weComXmlMessage.setChangeType("delete");
    ...
    private static String sendGroupChatEvent(String changeType) {
        WXBizMsgCrypt weComcpt = null;
        try {
            byte[] key = SecureUtil.generateKey(SymmetricAlgorithm.AES.getValue(), 256).getEncoded();
            String encode = Base64.encodeWithoutPadding(key);
    ...
    private void sendExternalUserEvent() {

    }


    private static String sendMemberEvent(String changeType) {
        WXBizMsgCrypt weComcpt = null;
        try {
            byte[] key = SecureUtil.generateKey(SymmetricAlgorithm.AES.getValue(), 256).getEncoded();
            String encode = Base64.encodeWithoutPadding(key);
    ...
    public static void main(String[] args) {
        String encryptMsg = sendGroupChatEvent("dismiss");//sendCorpTagEvent();
