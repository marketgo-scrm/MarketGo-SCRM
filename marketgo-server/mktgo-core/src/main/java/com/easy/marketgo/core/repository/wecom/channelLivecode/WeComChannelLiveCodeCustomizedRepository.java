...
public interface WeComChannelLiveCodeCustomizedRepository {
    List<WeComChannelLiveCodeEntity> listByBuildSqlParam(QueryChannelLiveCodeBuildSqlParam codeParam);

    Integer countByBuildSqlParam(QueryChannelLiveCodeBuildSqlParam codeParam);

    @Slf4j
    class WeComChannelLiveCodeCustomizedRepositoryImpl implements WeComChannelLiveCodeCustomizedRepository {
        @Resource
        private DataSource dataSource;

        @Override
        public List<WeComChannelLiveCodeEntity> listByBuildSqlParam(QueryChannelLiveCodeBuildSqlParam codeParam) {
            ...
            return new NamedParameterJdbcTemplate(this.dataSource).query(getSelectByCndSql(codeParam, false),
                    paramSource, new BeanPropertyRowMapper(WeComChannelLiveCodeEntity.class));
        }
        ...
        private String getSelectByCndSql(QueryChannelLiveCodeBuildSqlParam codeParam, boolean isCount) {
            StringBuilder sql = new StringBuilder(
                    String.format("SELECT %s FROM wecom_channel_live_code WHERE project_uuid = :projectUuid ",
                            isCount ? "COUNT(*)" : "*"));
