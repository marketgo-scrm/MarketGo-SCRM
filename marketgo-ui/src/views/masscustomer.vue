  ...
  <div>
    <main-head title="群发客户">
      <el-form class="head-form" inline size="small" label-width="0">
        <el-form-item>
          <el-button style="position: relative; top: -3px" type="primary" size="medium" icon="el-icon-circle-plus" round
            @click="toAdd">
            新建群发客户
          </el-button>
        </el-form-item>
        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="添加时间">
              <el-date-picker style="width: 100%" type="daterange" v-model="queryParams.times" align="right" unlink-panels
          ...
          <el-col :span="5">
            <el-form-item label="任务状态">
              <el-select style="width: 100%" multiple collapse-tags v-model="queryParams.statuses" placeholder="请选择"
                clearable>
                <el-option v-for="(dict, index) in statusesOption" :key="index" :label="dict.label" :value="dict.value" />
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
  ...
  methods: {
    ...
    stop(row) {
      ...
      this.$confirm('确定后未发送的员工将无法发送', '停止提示', {
        ...
        console.log(row)
        this.$http.post(
          `mktgo/wecom/mass_task/stop?corp_id=${this.$store.state.corpId}&project_id=${this.$store.state.projectUuid}&task_type=SINGLE&task_uuid=${row.uuid}`,
          {}).then(function (res) {
            console.log(res)
            if (res.code == 0) {
              _this.$message({
    ...
    remind(row) {
      let _this = this
      this.$confirm('确认后将重新触发通知, 提醒员工完成群发任务,24小时内每个任务最多触发三次提醒？', '提醒发送', {
        distinguishCancelAndClose: true,
        confirmButtonText: '确定',
        cancelButtonText: '取消',
      }).then(() => {
