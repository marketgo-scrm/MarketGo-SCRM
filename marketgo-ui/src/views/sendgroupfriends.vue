  ...
  methods: {
    ...
    del(row) {
      ...
      this.$confirm('是否确定删除？', '确认信息', {
        ...
        console.log(row)
        this.$http.post(
            `mktgo/wecom/mass_task/delete?project_id=${this.$store.state.projectUuid}&task_type=MOMENT&task_uuid=${row.uuid}`,
            {}).then(function (res) {
          console.log(res)
          if (res.code == 0) {
            _this.$message({
    ...
    remind(row) {
      let _this = this
      this.$http.post(
          `mktgo/wecom/mass_task/remind?corp_id=${this.$store.state.corpId}&project_id=${this.$store.state.projectUuid}&task_type=MOMENT&task_uuid=${row.uuid}`,
          {}).then(function (res) {
        console.log(res)
        if (res.code == 0) {
          _this.$message({
    ...
    async searchData(page_num) {
      ...
      let data = await this.$http.get(
          `mktgo/wecom/mass_task/list?corp_id=${this.$store.state.corpId}&project_id=${this.$store.state.projectUuid}&task_type=MOMENT`+queryStr,
          {});
      console.log(data)
      if (data.code == 0) {
    ...
    openDetails(row) {
      console.log(row)
      this.$router.push({
        path: '/index/sendgroupfriends-detail',
        query: {
          uuid: row.id,
          task_uuid: row.uuid,
    ...
    toAdd() {
      console.log(111)
      this.$router.push('/index/sendgroupfriends-add')
    },
    handleQuery() {
