<template>
  <div class="masscustomer-add">
    <main-head title="新建群发客户群" :back="true"></main-head>
    <div class="base-content">
      ...
      <el-form :model="baseForm" :rules="baseRules" ref="baseForm" label-width="100px" class="demo-baseForm">
        ...
        <el-form-item label="添加群主：" prop="members">
...
export default {
  ...
  methods: {
        ...
        if (valid) {
          ...
          this.$refs['setForm'].validate(async (valid) => {
            if (valid) {
              ...
              let data = await this.$http.post(`mktgo/wecom/mass_task/save?corp_id=${this.$store.state.corpId}&project_id=${this.$store.state.projectUuid}&task_type=GROUP`,
                  params);
              console.log(data)
    ...
    async checkName() {
      let data = await this.$http.get(`mktgo/wecom/mass_task/check_name?project_id=${this.$store.state.projectUuid}&task_name=${this.baseForm.name}&task_type=GROUP`,
          {});
      console.log(data)
      return data
    },
