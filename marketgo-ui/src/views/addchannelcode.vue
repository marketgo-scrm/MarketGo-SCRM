<template>
  <div>
    <main-head back :title="formData.id ? '编辑渠道活码' : '新建渠道活码'"></main-head>
    <el-form ref="ruleForm" size="small" :model="formData" :rules="rules" label-width="100px" @submit.prevent.native>
      <el-card class="box-card">
        <el-row :gutter="20">
          <el-col :sm="18" :md="14" :xs="24">
            <div class="custom-title">基础信息</div>
            <el-form-item label="二维码名称：" prop="name">
              <el-input v-model="formData.name" @blur="liveCodeCheckName()" maxlength="30"></el-input>
            </el-form-item>
            <el-form-item label="在线类型：" prop="onlineType">
              <el-radio v-model="formData.onlineType" :label="1" border>全天在线</el-radio>
            </el-form-item>
            <el-form-item required label="添加员工：">
              <div class="add-custom">
                ...
                <div class="add-custom-tag">
                  <CustomStretch :height="28" :standard="
                    formData?.members?.departments.length +
                    formData?.members?.users.length >
                    3
                  ">
                    <el-tag type="info" effect="plain" size="small" closable :disable-transitions="false"
                      @close="deleteDepartments(index)" v-for="(tag, index) in formData?.members?.departments"
                      :key="tag.id">
                      <img class="add-custom-tag-img" src="@/assets/avter.png" />
                      <span class="add-custom-tag-txt">{{ tag.name }}</span>
                    </el-tag>
                    <el-tag type="info" effect="plain" size="small" closable :disable-transitions="false"
                      @close="deleteUsers(index)" v-for="(tag, index) in formData?.members?.users" :key="tag.memberId">
                      <img class="add-custom-tag-img" src="@/assets/avter.png" />
              ...
              <custom-alert text="同一个二维码可选择多位员工进行接待，客户扫码后随机分配一名员工如果选择的是部门，若部门有新入职的员工，则自动添加为使用员工" />
              <span style="color: #F56C6C;font-size:12px;"
                v-if="formData?.members?.departments.length + formData?.members?.users.length <= 0">
                请添加员工
              </span>
            </el-form-item>
            <el-form-item label="添加客户上限：">
              <el-switch v-model="formData.addExtUserLimitStatus" active-color="#81DD9B"
                @change="extUserLimitStatusChange">
              </el-switch>
              <custom-alert text="开启后，员工从此活码添加的客户达到每日上线后，将自动下线当日不再接待该活码新客户" />
              <template v-if="formData.addExtUserLimitStatus">
                <div style="margin-top: 12px; font-size: 12px">
                  ...
                  <el-button :disabled="!addExtUserCeilings.length" @click="extUserCeilingOpen()" type="text">
                    设置上限
                  </el-button>
                </div>
                <el-checkbox-group v-model="addExtUserCeilings" v-if="formData.addExtUserLimit.length"
                  @change="handleCheckedCitiesChange">
                  <div class="custom-table" style="border-bottom: 0; border-radius: 4px 4px 0 0">
                  ...
                  <div style="
                    ...
                    <div class="custom-table" style="border: 0">
                      <div class="custom-table-tr" v-for="(item, index) in formData.addExtUserLimit" :key="index">
                        <div class="custom-table-td">
                          <el-checkbox :label="item.memberId">
                            <div style="display: grid">
                              <div class="user-info">
                                <img class="user-info-img" src="@/assets/avter.png" />
                        ...
                        <div class="custom-table-td">
                          <CustomNumber :max="999" :min="1" placeholder="请输入内容" :value="item.addExtUserCeiling"
                            @change="ceilingChange($event, index)" size="mini"></CustomNumber>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div class="add-custom">
                ...
                <div class="add-custom-tag">
                  <CustomStretch :height="28" :standard="
                    ...
                    3
                  ">
                    <el-tag type="info" effect="plain" size="small" closable :disable-transitions="false" @close="
                      formData?.backupMembers?.departments.splice(index, 1)
                    " v-for="(tag, index) in formData?.backupMembers
  ?.departments" :key="tag.id">
                      <img class="add-custom-tag-img" src="@/assets/avter.png" />
                      <span class="add-custom-tag-txt">{{ tag.name }}</span>
                    </el-tag>
                    <el-tag type="info" effect="plain" size="small" closable :disable-transitions="false"
                      @close="formData?.backupMembers?.users.splice(index, 1)"
                      v-for="(tag, index) in formData?.backupMembers?.users" :key="tag.memberId">
                      <img class="add-custom-tag-img" src="@/assets/avter.png" />
                      <span class="add-custom-tag-txt">{{
                        tag.memberName
                      }}</span>
              ...
              <custom-alert text="当员工都已达到添加上限或非上线时间，将分配客户给备用员工" />
            </el-form-item>
            <el-form-item label="客户标签：">
              <div class="add-custom">
                <div class="add-custom-btn">
                  <el-button @click="$refs.actiontagRef.open(formData.tags)" size="mini" icon="el-icon-plus"
                    circle></el-button>
                </div>
                <div class="add-custom-tag">
                  <CustomStretch :height="28" :standard="formData.tags.length > 3">
                    <el-tag type="info" effect="plain" size="small" closable :disable-transitions="false"
                      @close="formData.tags.splice(index, 1)" v-for="(tag, index) in formData.tags" :key="tag.id">
                      <span class="add-custom-tag-txt">{{ tag.name }}</span>
                    </el-tag>
                  </CustomStretch>
                </div>
        ...
        <el-row :gutter="50">
          <el-col :sm="18" :md="14" :xs="24">
            ...
            <el-form-item label="类型：">
              ...
              <custom-alert text="默认欢迎语：将发送员工已设置的欢迎语，若所选员工未设置欢迎语，则不会发送欢迎语" />
            </el-form-item>
              ...
              <el-form-item label="附件消息：">
                ...
                <template v-if="$route.query.uuid">
                  <template v-if="formData.id">
                    <EnclosureList ref="EnclosureList" :dataIn="EnclosureListIn" :callback="enclosureListCallback"
                      :type="'LIVE_CODE'"></EnclosureList>
                  </template>
                </template>
                <template v-else>
                  <EnclosureList ref="EnclosureList" :dataIn="EnclosureListIn" :callback="enclosureListCallback"
          ...
          <el-col :sm="6" :md="10" :xs="24" v-show="formData.welcomeType === 'USER'">
        ...
        <el-row :gutter="20">
          <el-col :sm="18" :md="14" :xs="24">
            ...
            <el-form-item label="自动通过好友：">
              ...
              <custom-alert text="开启后，客户添加该企业微信时无需验证，将会自动添加成功" />
            </el-form-item>
            <el-form-item label="二维码预览：">
              <div class="qrcode-img">
                <img class="qrcode-img-code" src="@/assets/slqrcode.png" />
                <img class="qrcode-img-logo" :src="`data:image/png;base64,${formData.logoMedia?.imageContent.replace(
                  /\\r\\n/g,
                  ''
              ...
              <div>
                <el-upload action :before-upload="giftImageUpdata" accept="image/*">
                  <el-button type="text" size="mini">更换二维码中间头像</el-button>
                </el-upload>
              </div>
              <div style="
                    color: #999999;
            ...
            <el-form-item>
              <el-button @click="$router.go(-1)" style="padding-left: 30px; padding-right: 30px" round>取消</el-button>
              <el-button style="padding-left: 30px; padding-right: 30px" round type="primary" @click="onSubmit">
    ...
    <!-- 设置客户上线 -->
    <el-dialog title="批量添加客户上线" :visible.sync="extUserCeilingVisible" width="300px" :before-close="extUserCeilingClose">
      <el-form size="small" @submit.prevent.native>
        <el-form-item required label="每日添加客户上线：">
          <CustomNumber :max="999" :min="1" placeholder="请输入内容" :value="addExtUserCeiling" @change="
            (e) => {
              addExtUserCeiling = e;
            }
          " size="mini"></CustomNumber>
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button size="small" @click="extUserCeilingClose" round>取 消</el-button>
        <el-button @click="extUserCeilingConfirm" size="small" round type="primary">确 定</el-button>
      </template>
    </el-dialog>
    <ActionTag ref="actiontagRef" @change="
      (e) => {
        formData.tags = e;
      }
    " />
...
export default {
  ...
  methods: {
    ...
    ruleCheck() {
      ...
      if (this.formData.welcomeType === 'USER') {
        let list = JSON.parse(JSON.stringify(this.formData.welcomeContent));
        list.find((item) => {
          if (item.type === "TEXT" && (!item.text.content || item.text.content.trim() === "")) {
            textIsEmpty = true
          }
        });
        ...
        if (textIsEmpty) {
          ...
          if (list.length === 0) {
            this.$message("欢迎语模块:请输入文本消息或附件消息");
            bool = false;
          }
        }
      }
