<template>
  <div class="masscustomer-add">
    <main-head title="新建客户群触达任务" :back="true"></main-head>
...
<!--            <custom-alert
                text="因企业微信限制，只可以群发消息给每个群主最近活跃（最近创建或最近发过消息）的200个群聊"
            />-->
          </div>
...
<!--                <div style="width: 100%">
              ...
              <el-form-item label="附件消息：" prop="radioFj" v-show="setForm.radioNr == 1">
                ...
                <EnclosureList ref="EnclosureList" :dataIn="EnclosureListIn" :limit="9" :callback="enclosureListCallback" :type="'GROUP'"></EnclosureList>
...
import condition from "@/components/condition-task.vue";
  ...
  data() {
    ...
    return {
      ...
      baseRules: {
        ...
        radioPc: [
          { required: true, message: '请选择是否排除客户群', trigger: 'change' }
        ],
      },
      setForm: {
        text: '111',
    ...
    submitBaseForm() {
      ...
      this.$refs['baseForm'].validate(async (valid) => {
        if (valid || !valid) {
          ...
          this.postDataBase.weComUserGroupRule.groupChats = this.$refs.condition.getData()
          this.conditionStr = JSON.stringify(this.postDataBase.weComUserGroupRule.groupChats)

          if (this.baseForm.radioPc == 1) {
            this.postDataBase.weComUserGroupRule.excludeSwitch = false
            this.postDataBase.weComUserGroupRule.excludeGroupChats = null
          } else {
            this.postDataBase.weComUserGroupRule.excludeSwitch = true
            this.postDataBase.weComUserGroupRule.excludeGroupChats = this.$refs.conditionPc.getData()
            this.conditionPcStr = JSON.stringify(this.postDataBase.weComUserGroupRule.excludeGroupChats)
          }
          ...
          let data = await this.$http.post(`mktgo/wecom/user_group/estimate?corp_id=${this.$store.state.corpId}&project_id=${this.$store.state.projectUuid}&request_id=${this.base_request_id}&task_type=GROUP`,
              params);
          console.log(data)
        ...
        if (valid || !valid) {
          ...
          this.$refs['setForm'].validate(async (valid) => {
            if (valid) {
              ...
              let data = await this.$http.post(`mktgo/wecom/task_center/save?corp_id=${this.$store.state.corpId}&project_id=${this.$store.state.projectUuid}&task_type=GROUP`,
                  params);
              console.log(data)
