        ...
        <el-form-item label="发送范围：" prop="name">
          ...
          <div v-if="rqShow" style="display: inline-block;position: relative">
            <div class="fjBox" style="padding: 0">
              <PreviewMemberDE :userGroupUuid="resData.userGroupUuid" :count="detailNum" :type="'SINGLE'"></PreviewMemberDE>
            </div>
          </div>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    remind() {
      let _this = this
      this.$http.post(
          `mktgo/wecom/mass_task/remind?corp_id=${this.$store.state.corpId}&project_id=${this.$store.state.projectUuid}&task_type=SINGLE&task_uuid=${this.$route.query.task_uuid}`,
          {}).then(function (res) {
            console.log(res)
        if (res.code == 0) {
          _this.$message({
