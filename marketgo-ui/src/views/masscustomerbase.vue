  ...
  methods: {
    ...
    del(row) {
      ...
      this.$confirm('是否确定删除？', '确认信息', {
        ...
        console.log(row)
        this.$http.post(
            `mktgo/wecom/mass_task/delete?project_id=${this.$store.state.projectUuid}&task_type=GROUP&task_uuid=${row.uuid}`,
            {}).then(function (res) {
          console.log(res)
          if (res.code == 0) {
            _this.$message({
    ...
    stop(row) {
      ...
      this.$confirm('确定后未发送的员工将无法发送？', '停止提示', {
        ...
        console.log(row)
        this.$http.post(
          `mktgo/wecom/mass_task/stop?corp_id=${this.$store.state.corpId}&project_id=${this.$store.state.projectUuid}&task_type=GROUP&task_uuid=${row.uuid}`,
          {}).then(function (res) {
            console.log(res)
            if (res.code == 0) {
              _this.$message({
    ...
    remind(row) {
      ...
      this.$confirm('确认后将重新触发通知, 提醒员工完成群发任务,24小时内每个任务最多触发三次提醒？', '提醒发送', {
        ...
        console.log(row)
        this.$http.post(
          `mktgo/wecom/mass_task/remind?corp_id=${this.$store.state.corpId}&project_id=${this.$store.state.projectUuid}&task_type=GROUP&task_uuid=${row.uuid}`,
          {}).then(function (res) {
            console.log(res)
            if (res.code == 0) {
              _this.$message({
    ...
    getCreators() {
      let _this = this
      this.$http.get(
          `mktgo/wecom/mass_task/creators?corp_id=${this.$store.state.corpId}&project_id=${this.$store.state.projectUuid}&task_type=GROUP`,
          {}).then(function (res) {
    ...
    async searchData(page_num) {
      ...
      let data = await this.$http.get(
          `mktgo/wecom/mass_task/list?corp_id=${this.$store.state.corpId}&project_id=${this.$store.state.projectUuid}&task_type=GROUP`+queryStr,
          {});
      console.log(data)
      if (data.code == 0) {
    ...
    openDetails(row) {
      console.log(row)
      this.$router.push({
        path: '/index/masscustomerbase-detail',
        query: {
          uuid: row.id,
          task_uuid: row.uuid,
    ...
    toAdd() {
      console.log(111)
      this.$router.push('/index/masscustomerbase-add')
    },
    handleQuery() {
