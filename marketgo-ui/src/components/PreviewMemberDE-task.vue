<template>
  <div class="PreviewMemberDE">
    ...
    <div class="content" v-if="type == 'GROUP'">
...
<!--          <el-form-item label="性别：" style="margin-top: 13px" v-if="externalUsers.genderSwitch">
            ...
            </section> style="margin-top: 15px;margin-bottom: 16px"
          </el-form-item>-->
          <el-form-item label="选择客户群：" v-if="externalUsers.groupChatsSwitch">
            <section v-for="(v,i) in externalUsers.groupChats" :key="i">{{ (i == 0 ? '' : ',') + v.cname }}</section>
          </el-form-item>
          <el-form-item label="客户群名称：" v-if="externalUsers.groupChatNameSwitch">
            <section>{{ externalUsers.groupChatName }}</section>
          </el-form-item>
          <el-form-item label="创建时间：" v-if="externalUsers.createTimeSwitch">
            <section>{{ externalUsers.startTime }}  <span>至</span> {{ externalUsers.endTime }}</section>
          </el-form-item>
          <el-form-item label="群人数：" v-if="externalUsers.userCountSwitch" style="margin-top: 15px;margin-bottom: 16px">
            <section>{{ getRule(externalUsers.userCountRule) }} {{ externalUsers.userCount }}</section>
          </el-form-item>
...
&lt;!&ndash;              <div v-for="(v,i) in externalUsers.corpTags.tags" :key="i">{{ v.cname }}</div>&ndash;&gt;
              &lt;!&ndash;              <div>每日数据观察</div>&ndash;&gt;
...
&lt;!&ndash;            <section>{{externalUsers.corpTags.relation == 'OR'?'满足任意一个标签的客户':'满足所有标签的客户'}}</section>&ndash;&gt;
...
export default {
  ...
  mounted() {
    ...
    this.$http.get(
      ...
      _this.externalUsers = _this.resData.userGroup.weComUserGroupRule.groupChats
      _this.formData.members = _this.resData.userGroup.weComUserGroupRule.members
    })
  },
  methods: {
    getRule(type) {
      // GT 大于，GTE 大于或等于，LT 小于，LTE 小于或等于，EQ 等于 NEQ 不等于
      if (type == 'GT') {
        return '大于'
      }
      if (type == 'GTE') {
