  ...
  <div class="condition">
    <div class="style-tab">
      <div class="style-item">
        <el-checkbox v-model="checked1" >选择客户群</el-checkbox>
      </div>
      <div class="style-item">
        <el-checkbox v-model="checked2" >客户群名称</el-checkbox>
      </div>
      <div class="style-item">
        <el-checkbox v-model="checked3" >创建时间</el-checkbox>
      </div>
      <div class="style-item">
        <el-checkbox v-model="checked4" >群人数</el-checkbox>
      </div>
    </div>
    ...
    <div class="content">
      ...
      <div class="right">
        <el-form ref="baseForm" class="demo-baseForm" label-width="85px">
          <el-form-item label="选择客户群：" v-show="checked1">
<!--            <el-select v-model="value" placeholder="" style="width: 374px">
          ...
          <el-form-item label="客户群名称：" v-show="checked2" style="margin-top: 13px;margin-bottom: 13px">
<!--            <div class="style-tab-radio">
          ...
          <el-form-item label="群人数：" v-show="checked4" style="margin-top: 15px;margin-bottom: 16px">
<!--            <el-select v-model="valueGroup" multiple placeholder="请选择" style="width: 374px">
              <el-option
                  v-for="item in optionsGroup"
                  :key="item.groupChatId"
            ...
            <el-select v-model="externalUsers.userCountRule" placeholder="" style="width: 182px">
              <el-option
                  v-for="item in optionsUserCountRule"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value">
              </el-option>
            </el-select>
            <el-select v-model="externalUsers.userCount" placeholder="" style="width: 182px;margin-left: 13px">
              <el-option
                  v-for="item in optionsUserCount"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value">
              </el-option>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      optionsUserCountRule: [{//GT 大于，GTE 大于或等于，LT 小于，LTE 小于或等于，EQ 等于 NEQ 不等于
        value: 'GT',
        label: '大于'
      }, {
        value: 'GTE',
      ...
      "externalUsers": {
        ...
        "groupChats": null,
        "userCount": 10,
        "userCountRule": 'EQ',//客户群人数的规则， GT 大于，GTE 大于或等于，LT 小于，LTE 小于或等于，EQ 等于 NEQ 不等于
        "userCountSwitch": false,
  ...
  methods: {
    getData() {
      this.externalUsers.createTimeSwitch = this.checked3
      this.externalUsers.groupChatNameSwitch = this.checked2
      this.externalUsers.groupChatsSwitch = this.checked1
      this.externalUsers.userCountSwitch = this.checked4
      ...
      this.externalUsers.startTime = this.externalUsers.createTimeSwitch && this.startTime ? this.startTime[0] : null
      this.externalUsers.endTime = this.externalUsers.createTimeSwitch && this.startTime ? this.startTime[1] : null
